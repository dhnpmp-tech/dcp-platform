...
const PROOF_PREFIX = 'adapter-vllm-live-load-proof';
const CONTRACT = 'dcp.adapter_vllm_live_load_proof.v1';
...
function classifyFailure(code, message, details = {}) {
  const actions = {
    LIVE_PROOF_NOT_ENABLED: 'Set DCP_ADAPTER_VLLM_LIVE_PROOF_ALLOW=1 only when you intend to run live adapter/vLLM readiness and future endpoint checks.',
    SMOKE_PRINCIPAL_FAILED: 'Ensure the deterministic smoke principal exists and can authenticate against LoRA/adapter readiness routes.',
...
function summarizeReadiness(body) {
  ...
  return {
    ...
    endpoints: readiness.endpoints && typeof readiness.endpoints === 'object' ? { ...readiness.endpoints } : {},
    adapter_registry: {
      status: adapterRegistry.status || null,
      api_available: adapterRegistry.api_available ?? null,
      public_upload_enabled: adapterRegistry.public_upload_enabled ?? null,
      ...
      next: adapterRegistry.next || null,
    },
    adapter_deployments: {
      status: adapterDeployments.status || null,
      api_available: adapterDeployments.api_available ?? null,
      modes: Array.isArray(adapterDeployments.modes) ? [...adapterDeployments.modes] : [],
      ...
      next: adapterDeployments.next || null,
    },
    claim_guards: {
...
function findReadinessBlockers(readiness) {
  ...
  if (readiness.adapter_registry.api_available !== true) blockers.push('adapter_registry.api_available');
  if (readiness.adapter_registry.public_upload_enabled !== true) blockers.push('adapter_registry.public_upload_enabled');
  if (readiness.adapter_registry.serving_enabled !== true) blockers.push('adapter_registry.serving_enabled');
  if (readiness.adapter_registry.route_traffic !== true) blockers.push('adapter_registry.route_traffic');
  if (readiness.adapter_registry.checksum_required !== true) blockers.push('adapter_registry.checksum_required');
  if (readiness.adapter_deployments.api_available !== true) blockers.push('adapter_deployments.api_available');
  if (readiness.adapter_deployments.serving_enabled !== true) blockers.push('adapter_deployments.serving_enabled');
  if (readiness.adapter_deployments.route_traffic !== true) blockers.push('adapter_deployments.route_traffic');
  if (readiness.adapter_deployments.load_proof_required !== false) blockers.push('adapter_deployments.load_proof_required');
  if (readiness.claim_guards.public_serving_enabled !== true) blockers.push('claim_guards.public_serving_enabled');
  if (readiness.claim_guards.route_traffic !== true) blockers.push('claim_guards.route_traffic');
...
function buildMarkdown(report) {
  const lines = [];
  lines.push('# Adapter vLLM Live Load Proof');
  lines.push('');
  ...
  lines.push('checks the live renter-authenticated LoRA/adapter readiness contract and stops');
  lines.push('before creating adapters, creating deployments, posting internal vLLM load');
  lines.push('proofs, routing adapter traffic, calling a dedicated endpoint, writing billing');
  lines.push('ledger entries, enabling discounts, or making Tinker/multi-LoRA claims. A');
  lines.push('future PASS must include adapter checksum, vLLM load proof, endpoint response,');
  lines.push('and adapter billing evidence.');
  lines.push('');
  return `${lines.join('\n')}\n`;
}
...
async function runAdapterVllmLiveLoadProof(options = {}) {
  const baseUrl = normalizeBaseUrl(options.baseUrl);
  const outputDir = path.resolve(options.outputDir || process.env.DCP_ADAPTER_VLLM_LIVE_PROOF_OUTPUT_DIR || OUTPUT_DIR_DEFAULT);
  ...
  try {
    if (process.env.DCP_ADAPTER_VLLM_LIVE_PROOF_ALLOW !== '1' && options.allowLive !== true) {
      throw Object.assign(new Error('Live adapter vLLM load proof is disabled by default'), {
    ...
    const readinessRes = await requestJson(baseUrl, '/api/lora/readiness', {
      headers,
      fetchImpl,
      timeoutMs: Number.parseInt(process.env.DCP_ADAPTER_VLLM_LIVE_PROOF_TIMEOUT_MS || '120000', 10),
    });
    ...
    if (!readinessRes.ok) {
      throw Object.assign(new Error(`LoRA/adapter readiness request failed with HTTP ${readinessRes.status}`), {
    ...
    if (report.readiness.object !== 'lora_readiness'
      ...
      || report.readiness.adapter_registry.api_available !== true
      || report.readiness.adapter_deployments.api_available !== true) {
      throw Object.assign(new Error('LoRA/adapter readiness contract is not internally consistent enough for live adapter load proof'), {
    ...
    if (blockers.length > 0) {
      throw Object.assign(new Error('Adapter vLLM load, route traffic, endpoint smoke, and billing proof is not enabled by readiness'), {
    ...
    throw Object.assign(new Error('Readiness claims live adapter serving, but adapter create/deployment/load-proof/endpoint/billing proof steps are not implemented in this runner yet'), {
      code: 'ADAPTER_VLLM_LIVE_FLOW_NOT_IMPLEMENTED',
      details: {
        readiness: report.readiness,
      },
    });
  } catch (error) {
    report.failure = classifyFailure(error.code || 'ADAPTER_VLLM_LIVE_LOAD_PROOF_FAILED', error.message, error.details || null);
    if (error.code && !['LIVE_PROOF_NOT_ENABLED', 'ADAPTER_VLLM_LOAD_NOT_ENABLED'].includes(error.code)) {
      report.verdict = 'FAIL';
    }
    return {
      report,
      exitCode: ['LIVE_PROOF_NOT_ENABLED', 'ADAPTER_VLLM_LOAD_NOT_ENABLED'].includes(error.code) ? 2 : 1,
      transcript,
    };
  } finally {
...
if (require.main === module) {
  main().catch((error) => {
    process.stderr.write(`${JSON.stringify({
      verdict: 'error',
      code: error.code || 'ADAPTER_VLLM_LIVE_LOAD_PROOF_ERROR',
