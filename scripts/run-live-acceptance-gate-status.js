...
const LIVE_ACCEPTANCE_GATES = Object.freeze([
  ...
  },
  {
    id: 'openai_sse_live',
    lane: 'Inference',
    product_area: 'OpenAI-compatible Chat Completions SSE path',
    acceptance_state: 'blocked',
    acceptance_command: 'DCP_OPENAI_SSE_PROOF_ALLOW_LIVE=1 npm run proof:openai-sse',
    command_available: true,
    artifact_pattern: 'docs/reports/reliability/openai-sse-live-proof-*.json',
    blocked_on: ['funded inference smoke principal', 'compatible vLLM provider capacity'],
    verifies: ['POST /v1/chat/completions', 'text/event-stream preservation', 'OpenAI delta frames and data: [DONE]'],
    claim_guard: 'Do not claim OpenAI-compatible streaming accepted until a funded live proof passes.',
    next_action: 'Run the live proof when the smoke principal has balance and compatible capacity exists.',
  },
  {
  ...
  },
  {
    ...
    next_action: 'Run the opt-in live load proof during an adapter/vLLM/funded window; it remains blocked until readiness proves load, routing, endpoint smoke, and billing.',
  },
  {
