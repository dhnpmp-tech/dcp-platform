...
| Adapter billing approval readiness | `npm run proof:adapter-billing-approval` | none | CI-safe gate available; validates evidence-packet hash, local-roadmap proof, production smoke, and founder signoff prerequisites while approval mutation, adapter billing, invoices, payouts, route changes, balance mutations, and Tinker claims remain disabled |
...
| Prompt-cache live hit/settlement smoke | `DCP_PROMPT_CACHE_LIVE_PROOF_ALLOW=1 npm run proof:prompt-cache-live-settlement` | provider cache-hit evidence, funded smoke principal, settlement discount policy approval | Command available; readiness now exposes `dcp.prompt_cache_live_acceptance_evidence.v1`; a PASS proves live miss/hit measurement and no-discount guards only, with provider KV-cache control and discounted settlement still gated |
...
| Batch live execution/discount smoke | `DCP_BATCH_LIVE_PROOF_ALLOW=1 npm run proof:batch-live-execution` | funded smoke principal, object-store result path, live provider execution capacity, discount policy approval | Command available; readiness now exposes `dcp.batch_live_acceptance_evidence.v1`; a PASS requires readiness, create, poll, result manifest, checksum download, per-line execution, discounted settlement, and model capability evidence |
...
| Adapter vLLM load/billing smoke | `DCP_ADAPTER_VLLM_LIVE_PROOF_ALLOW=1 npm run proof:adapter-vllm-live-load` | real adapter artifact checksum, vLLM host with LoRA enabled, dedicated endpoint capacity, funded smoke principal, deployment intent, endpoint-smoke evidence, usage attribution, billing policy approval | Command available; readiness and billing packets now expose `dcp.adapter_vllm_live_acceptance_evidence.v1`; a PASS requires readiness claims, funded principal, adapter checksum, deployment intent, strict vLLM load proof, endpoint smoke, usage attribution, billing policy, and claim-boundary evidence before adapter serving, route traffic, endpoint smoke, billing, or dedicated-deployment claims can turn true |
...
7. **Adapter deployment and dedicated endpoints**
   - Gate: `proof:adapter-deployment-contract`,
     ...
     `proof:adapter-billing-approval`, `proof:adapter-billing-readiness`, and
...
- Adapter endpoint smoke disabled submission: run
  ...
  return a no-record 409 while disabled, and avoid exposing raw prompt/response
  content.
...
- Adapter billing approval readiness: run
  `npm run proof:adapter-billing-approval` before any approval mutation,
  adapter billing enablement, invoice, payout, balance mutation, or billed
  dedicated-endpoint claim.
