...
const NAV: NavSection[] = [
  ...
  },
  {
    ...
    secAr: 'الحساب',
    items: [
      { k: 'profile', ic: '✦', label: 'Profile', labelAr: 'الملف الشخصي', href: '/provider/profile' },
      { k: 'settings', ic: '⚙', label: 'Settings', labelAr: 'الإعدادات', href: '/provider/settings' },
      { k: 'docs', ic: '?', label: 'Provider docs', labelAr: 'وثائق المزوّد', href: '/docs', bd: '↗' },
    ],
  },
]
        ...
        <header className="pv-tb" id="pv-tb" data-crumb="Earnings">
          ...
          <button
            ...
            onClick={async () => {
              const key = getProviderKey()
              if (!key) {
                window.location.href = '/auth?role=provider&redirect=/provider/earnings'
                return
              }
        ...
        <main className="pv-main">
          ...
          {dataState === 'missing-key' && (
            <div className="dash-state err" style={{ marginTop: 24 }}>
              ...
              <Link href="/auth?role=provider&method=apikey&redirect=/provider/earnings">
