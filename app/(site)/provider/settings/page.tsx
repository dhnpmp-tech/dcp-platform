      ...
      <aside className={`pv-sb${drawerOpen ? ' on' : ''}`} id="pv-sb" data-page="settings">
        ...
        <div className="pv-sb-foot">
          ...
          <span
            ...
            onClick={() => { localStorage.removeItem('dc1_provider_key'); window.location.href = '/auth?role=provider' }}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { localStorage.removeItem('dc1_provider_key'); window.location.href = '/auth?role=provider' } }}
          >↱</span>
        </div>
      </aside>
        ...
        <main className="pv-main">
          ...
          {loadState === 'missing-key' && (
            <div className="dash-state err" style={{ marginTop: 24 }}>
              ...
              <Link href="/auth?role=provider&method=apikey&redirect=/provider/settings">
