...
function AuthInner() {
  ...
  const defaultRedirect = useMemo(() => {
    if (tab === 'signup' && role === 'renter') return '/setup'
    if (tab === 'signup' && role === 'provider') return '/provider-setup'
    return role === 'provider' ? '/provider/dashboard' : '/renter/dashboard'
  }, [role, tab])
  ...
  const adminAuthHref = '/auth?role=admin&method=apikey&redirect=/admin'
