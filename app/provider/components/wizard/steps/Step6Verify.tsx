...
export function Step6Verify({ apiKey, onBack, onDone }: Step6Props) {
  ...
  return (
    <div className="space-y-5 rounded-2xl border border-dc1-border bg-dc1-surface-l1 p-6 md:p-8">
      ...
      <div className="flex items-center gap-4 rounded-lg border border-dc1-border bg-dc1-surface-l2 p-4">
        ...
        <div className="flex-1 text-sm">
          <p className="font-semibold text-dc1-text-primary">
            Status: {status?.status === 'registered' ? 'Registered, waiting for daemon' : 'Polling…'}
          </p>
          <p className="text-xs text-dc1-text-muted">
            Checking every 5 seconds ({elapsed}s elapsed)
          </p>
        </div>
      </div>
      ...
      <details className="rounded-lg border border-dc1-border bg-dc1-surface-l2 p-4 text-sm">
        ...
        <ul className="mt-3 space-y-1.5 text-xs text-dc1-text-secondary">
          <li>• Command didn&apos;t run? Go back one step to re-show the install command.</li>
          <li>• Error in terminal? Check that you ran it as Administrator (Windows) / sudo (Linux/macOS).</li>
          <li>• Taking too long? Verify your firewall allows outbound HTTPS to api.dcp.sa.</li>
          <li>• Still stuck after a minute? Contact support from the link below.</li>
        </ul>
      </details>
