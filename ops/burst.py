...
BLACKWELL_NEEDLES = ("blackwell", "b200", "rtx pro 6000", "rtx pro 4500", "5090")
...
# cu128/Blackwell images ship torch only in the venv python interpreter, not python3;
# mirror it so python3 finds torch like every other card. No-op on cu121 images where python3 already has torch.
PYV="$(command -v python 2>/dev/null)"; if [ -n "$PYV" ] && "$PYV" -c "import torch" >/dev/null 2>&1 && ! python3 -c "import torch" >/dev/null 2>&1; then ln -sf "$PYV" /usr/local/bin/python3; fi
