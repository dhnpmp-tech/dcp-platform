...
test('renter pods launch keeps workspace compact and compute selection explicit', async ({ page, context }) => {
  ...
  const reviewMap = page.getByLabel('Workspace decision map');
  await expect(reviewMap).toContainText('Fast review');
  await expect(reviewMap).toContainText('Files, GPU, launch — in order');
  await expect(reviewMap).toContainText('Stage 1');
  await expect(reviewMap).toContainText('Workspace collapsed by folders');
  await expect(reviewMap).toContainText('5 files · 3 folders');
  await expect(reviewMap).toContainText('Open one folder only when the summary is not enough.');
  await expect(reviewMap.getByRole('button', { name: /Open datasets\// })).toBeVisible();
  await expect(reviewMap).toContainText('Stage 2');
  await expect(reviewMap).toContainText('Auto-pick GPU');
  await expect(reviewMap).toContainText('gpu_type omitted = auto-pick');
  await expect(reviewMap).toContainText('Suggested: RTX 4090. Memory chips are browse filters, not a launch slider.');
  await expect(reviewMap.getByRole('link', { name: 'Review GPU' })).toBeVisible();
  await expect(reviewMap.getByRole('button', { name: 'Use suggested GPU' })).toBeVisible();
  await expect(reviewMap).toContainText('Stage 3');
  await expect(reviewMap).toContainText('PyTorch · 1h');
  await expect(reviewMap).toContainText('No trial-account tag live');
  await expect(reviewMap).toContainText('Trial route: DCP/community GPU pool; High-demand GPUs: paid credit only.');
  await expect(reviewMap.getByRole('link', { name: 'Confirm launch' })).toBeVisible();
  ...
  await expect(reviewMap).toContainText('RTX 4090');
  await expect(reviewMap).toContainText('gpu_type = RTX 4090');
  await expect(reviewMap).not.toContainText('Use suggested GPU');
